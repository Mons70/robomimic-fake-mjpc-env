...
def get_observations_from_states(state, obs_size):
    ret = {}
    ret["pos"] = np.array(state[:obs_size]) # fetch position indexes from state array
    ret["vel"] = np.array(state[obs_size:]) # fetch velocity indexes from state array
...
def extract_trajectory(
    ...
    obs = get_observations_from_states(list(initial_state.values())[0], obs_size)
    ...
    for t in range(1, traj_len + 1):
        ...
        if t == traj_len:
            ...
            next_obs = get_observations_from_states(states[t-1], obs_size) #KLUDGE (woooow, Shin)
        else:
            # reset to simulator state to get observation
            #next_obs = env.reset_to({"states" : states[t]})
            next_obs = get_observations_from_states(states[t], obs_size)
...
if __name__ == "__main__":
    ...
    # Specifies where to split the state into qpos and qvel observations 
    parser.add_argument(
        "--obs_size",
        type=str,
        help="Size of qpos observation in a state, to know where to split the state into qpos and qvel observations"
    )
